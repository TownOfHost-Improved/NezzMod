...
using NEZZ.Roles.Modifiers.Crewmate;
...
public static class Utils
{
    ...
    public static string GetSubRolesText(byte id, bool disableColor = false, bool intro = false, bool summary = false)
    {
        ...
        foreach (var role in SubRoles.ToArray())
        {
            ...
            if (summary && role is CustomRoles.Madmate or CustomRoles.CorruptedA or CustomRoles.Charmed or CustomRoles.Shaded or CustomRoles.Recruit or CustomRoles.Admired or CustomRoles.Darkened or CustomRoles.Infected or CustomRoles.Contagious or CustomRoles.Soulless or CustomRoles.Enchanted) continue;
    ...
    public static void SendMessage(string text, byte sendTo = byte.MaxValue, string title = "", bool logforChatManager = false, bool noReplay = false, bool ShouldSplit = false)
    {
        ...
        if (!logforChatManager)
            ChatManager.AddNEZZstMessage(text.RemoveHtmlTagsTemplate());
    ...
    {
        return Path.Combine(Main.NEZZData, "Tags", "VIP_TAGS", $"{friendCode}.txt");
    }

    public static string GetVipNameFile(string friendCode)
    {
        return Path.Combine(Main.NEZZData, "Tags", "VIP_TAGS", $"{friendCode}-name.txt");
    }

    public static string GetModTagsFile(string friendCode)
    {
        return Path.Combine(Main.NEZZData, "Tags", "MOD_TAGS", $"{friendCode}.txt");
    }

    public static string GetModNameFile(string friendCode)
    {
        return Path.Combine(Main.NEZZData, "Tags", "MOD_TAGS", $"{friendCode}-name.txt");
    }
    ...
    public static void ApplySuffix(PlayerControl player)
    {
        ...
        if (player.AmOwner)
        {
            ...
            {
                SuffixModes.NEZZ => name += $"\r\n<color={Main.ModColor}>NEZZ v{Main.PluginDisplayVersion + Main.PluginDisplaySuffix}</color>",
                ...
                SuffixModes.AuNEZZst => name += $"\r\n<size=1.7><color={Main.ModColor}>{GetString("SuffixModeText.AuNEZZst")}</color></size>",
                _ => name
            };
        }
    ...
    {
        string f = $"{Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory)}/NEZZ-logs/";
