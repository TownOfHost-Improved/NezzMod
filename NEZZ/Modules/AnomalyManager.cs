    ...
    public static void Holiday()
    {
        foreach (var player in Main.AllAlivePlayerControls)
        {
            ...
            if (role.IsImpostor())
            {
                ...
                player.RpcChangeRoleBasis(CustomRoles.ImpostorNEZZ);
                player.RpcSetCustomRole(CustomRoles.ImpostorNEZZ);
            }
            if (role.IsCoven()) return;
            if (role.IsNeutral()) return;
            if (role.IsCrewmate())
            {
                FormerRoles[player.PlayerId] = role;
                player.RpcChangeRoleBasis(CustomRoles.CrewmateNEZZ);
                player.RpcSetCustomRole(CustomRoles.CrewmateNEZZ);
            }
