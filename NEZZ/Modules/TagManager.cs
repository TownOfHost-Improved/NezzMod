...
{
    private static readonly string TAGS_FILE_PATH = Path.Combine(Main.NEZZData, "Tags");
    ...
    public static bool CheckFriendCode(string friendCode, bool log = false)
    {
        var folderPaths = Directory.GetDirectories(TAGS_FILE_PATH)
            .Where(folder => !new[] { "MOD_TAGS", "VIP_TAGS", "SPONSOR_TAGS", "EXCLUSIVE_TAGS", "SPECIAL_TAGS" }.Contains(Path.GetFileName(folder)))  // Ignore specific folders
            .ToList();
