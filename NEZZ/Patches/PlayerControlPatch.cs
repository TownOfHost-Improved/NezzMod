...
public static class CheckShapeshiftPatch
{
    ...
    private static bool CheckInvalidShapeshifting(PlayerControl instance, PlayerControl target, bool animate)
    {
        ...
        {
            if (!(instance.Is(CustomRoles.ShapeshifterNEZZ) || instance.Is(CustomRoles.Shapeshifter)) && target.GetClient().GetHashedPuid() == Main.FirstDiedPrevious && MeetingStates.FirstMeeting)
            {
...
class PlayerControlLocalSetRolePatch
{
    public static void Postfix(PlayerControl __instance, [HarmonyArgument(0)] RoleTypes role)
    {
        if (!AmongUsClient.Instance.AmHost && GameStates.IsNormalGame && !GameStates.IsModHost)
        {
            ...
            {
                RoleTypes.Crewmate => CustomRoles.CrewmateNEZZ,
                RoleTypes.Impostor => CustomRoles.ImpostorNEZZ,
                RoleTypes.Scientist => CustomRoles.ScientistNEZZ,
                RoleTypes.Engineer => CustomRoles.EngineerNEZZ,
                RoleTypes.Shapeshifter => CustomRoles.ShapeshifterNEZZ,
                RoleTypes.Noisemaker => CustomRoles.NoisemakerNEZZ,
                RoleTypes.Phantom => CustomRoles.PhantomNEZZ,
                RoleTypes.Tracker => CustomRoles.TrackerNEZZ,
                RoleTypes.Viper => CustomRoles.ViperNEZZ,
                RoleTypes.Detective => CustomRoles.DetectiveNEZZ,
