        ...
        if (AmongUsClient.Instance.AmHost) // Execute the following only on the Host
        {
            ...
            switch (GameOptionsManager.Instance.CurrentGameOptions.GameMode)
            {
                ...
                case GameModes.HideNSeek:
                    ...
                    // If custom Gamemode is Standard/FFA/C&R in H&S game, set HideNSeekNEZZ
                    if (Options.CurrentGameMode is CustomGameMode.Standard or CustomGameMode.FFA or CustomGameMode.CandR or CustomGameMode.FourCorners or CustomGameMode.UltimateTeam)
                    {
