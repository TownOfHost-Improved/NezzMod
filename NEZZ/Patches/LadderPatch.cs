...
public class FallFromLadder
{
    ...
    public static void FixedUpdate(PlayerControl player)
    {
        ...
        if (TargetLadderData.TryGetValue(player.PlayerId, out var ladderData))
        {
            if (Utils.GetDistance(ladderData, player.transform.position) < 0.5f)
            {
                ...
                _ = new LateTask(() =>
                {
                    Vector2 targetPos = (Vector2)ladderData + new Vector2(0.1f, 0f);
