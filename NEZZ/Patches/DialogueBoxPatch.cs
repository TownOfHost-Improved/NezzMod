...
internal class DialogueBoxPatch
{
    ...
    {
        if (!PlayerControl.LocalPlayer.inVent && dialogue.Contains("<size=0%>NEZZ</size>") && GameStates.IsInTask)
        {
    ...
    {
        if (__instance.target.text.Contains("<size=0%>NEZZ</size>"))
        {
    ...
    /*
     * Lets add a <size=0%>NEZZ</size> at where you will use the dialogue box.
     */
}
