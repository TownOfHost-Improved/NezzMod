...
public static class ToggleOptionPatch
{
    ...
    // Thanks Gurge44 - https://github.com/Gurge44/EndlessHostRoles
...
public static class StringOptionPatch
{
    ...
    private static void SetupHelpIcon(CustomRoles role, StringOption __instance)
    {
        ...
        GameOptionsButton.OnClick.AddListener((UnityEngine.Events.UnityAction)(() =>
        {
            ...
            if (ModGameOptionsMenu.OptionList.TryGetValue(__instance, out var index))
            {
                ...
                {
                    var roleName = role.IsVanilla() ? role + "NEZZ" : role.ToString();
        ...
        GameOptionsButton.interactableColor = Utils.GetRoleColor(role);
    ...
    private static bool UpdateValuePrefix(StringOption __instance)
    {
        if (ModGameOptionsMenu.OptionList.TryGetValue(__instance, out var index))
        {
            ...
            {
                if (Options.CurrentGameMode == CustomGameMode.HidenSeekNEZZ && !GameStates.IsHideNSeek) //Hide & Seek
                {
                    Options.GameMode.SetValue(0);
                }
                else if (Options.CurrentGameMode == CustomGameMode.HidenSeekNEZZ && GameStates.IsHideNSeek)
                {
                    Options.GameMode.SetValue(Options.GetGameModeInt(CustomGameMode.HidenSeekNEZZ));
                }
