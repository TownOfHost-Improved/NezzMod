...
{
    private static readonly string modLogFiles = Path.Combine(Main.NEZZData, "ModLogs.txt");
    private static readonly string modTagsFiles = Path.Combine(Main.NEZZData, "Tags", "MOD_TAGS");
    private static readonly string sponsorTagsFiles = Path.Combine(Main.NEZZData, "Tags", "SPONSOR_TAGS");
    private static readonly string vipTagsFiles = Path.Combine(Main.NEZZData, "Tags", "VIP_TAGS");
    private static readonly string VIPListPath = Path.Combine(Main.NEZZData, "VIP-List.txt");
    private static readonly string ModListPath = Path.Combine(Main.NEZZData, "Moderators.txt");
    ...
    public static string FixRoleNameInput(string text)
    {
        ...
        return text switch
        {
            ...
            // 原版职业
            "船員" or "船员" or "白板" or "天选之子" => GetString("CrewmateNEZZ"),
            "工程師" or "工程师" => GetString("EngineerNEZZ"),
            "科學家" or "科学家" => GetString("ScientistNEZZ"),
            "守護天使" or "守护天使" => GetString("GuardianAngelNEZZ"),
            "偽裝者" or "内鬼" => GetString("ImpostorNEZZ"),
            "變形者" or "变形者" => GetString("ShapeshifterNEZZ"),
            ...
            if (role == roleName.ToLower().Trim().TrimStart('*').Replace(" ", string.Empty))
            {
                ...
                if ((isDev || isUp) && GameStates.IsLobby)
                {
                    ...
                    if (rl.IsGhostRole() && !rl.IsAdditionRole() && isDev && (rl.GetCount() >= 1 && rl.GetMode() > 0))
                    {
                        ...
                        {
                            Custom_Team.Impostor => CustomRoles.ImpostorNEZZ,
                            _ => CustomRoles.CrewmateNEZZ
...
internal class UpdateCharCountPatch
{
    public static void Postfix(FreeChatInputField __instance)
    {
        ...
        __instance.charCountText.SetText(length <= 0 ? GetString("ThankYouForUsingNEZZ") : $"{length}/{__instance.textArea.characterLimit}");
