...
internal class DoubleAgent : RoleBase
{
    ...
    [
        0, // NoChange
        0, // Random
        CustomRoles.ImpostorNEZZ, // Team Crewmate
    ...
    public override void OnFixedUpdate(PlayerControl player, bool lowLoad, long nowTime, int timerLowLoad)
    {
        ...
        if (ChangeRoleToOnLast.GetValue() != 0 && StartedWithMoreThanOneImp && !GameStates.IsExilling && !AntiBlackout.SkipTasks)
        {
            if (player.Is(CustomRoles.DoubleAgent) && player.IsAlive() && Main.AliveImpostorCount < 2)
            {
                ...
                // If Role is ImpostorNEZZ aka Admired Impostor opt give Admired Modifier if player dose not already have it
                if (Role == CustomRoles.ImpostorNEZZ && !player.GetCustomSubRoles().Contains(CustomRoles.Admired))
                ...
                if (Role == CustomRoles.ImpostorNEZZ)
                    RoleName = ColorString(GetRoleColor(CustomRoles.Admired), $"{GetString("Admired")} {GetString("ImpostorNEZZ")}");
